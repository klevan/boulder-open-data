...
```{r libraries functions & paths, echo=FALSE}
rm(list = ls())
...
# Set parameters
bbox <- c(min(address.dbf$LONGITUDE),min(address.dbf$LATITUDE),
          max(address.dbf$LONGITUDE)+.1,max(address.dbf$LATITUDE))
boulder.map <- get_map(location = bbox, maptype = "terrain")
...
```{r community mapping 2, echo=FALSE}
# Set parameters
bbox <- c(min(address.dbf$LONGITUDE),min(address.dbf$LATITUDE),
          max(address.dbf$LONGITUDE)+.1,max(address.dbf$LATITUDE))
boulder.map <- get_map(location = bbox, maptype = "terrain")
...
OSMP.df %>% 
  filter(lat > bbox[2], lat < bbox[4],
         long > bbox[1], long < bbox[3],PUBLICACCE=="y") -> OSMP.df.y
OSMP.df %>% 
  filter(lat > bbox[2], lat < bbox[4],
         long > bbox[1], long < bbox[3],PUBLICACCE!="y") -> OSMP.df.n

ggmap(boulder.map, extent = "normal", darken = 0.2) + 
  geom_polygon(data = OSMP.df.y, aes(x = long, y = lat,group=group), 
               fill = "brown", alpha = 0.4) +
  geom_polygon(data = OSMP.df.n, aes(x = long, y = lat,group=group), 
               fill = "black") +
  geom_density2d(data = trees.dbf, aes(x = Longitude, y = Latitude), 
                 color = "darkgreen") +
  ylab("Latitude") +
